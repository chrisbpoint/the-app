...
class AppTool:
    ...
    def __init__(self, concurrency_service, io_service, metrics_service):
        self._x_tool = XTool(io_service.image_data)
        self._y_tool = YTool(io_service.adc_data)
        self._logger_tool = LoggerTool(metrics_service.metrics_data)
    ...
    def _create_app_tool_ui_connections(self):
        ...
        self._app_tool_ui.tab_widget.currentChanged.connect(self._update_offline)
    ...
    def _create_logger_tool_connections(self):
        self._y_tool.ui.start_button.clicked.connect(self._logger_tool.initialize)
    ...
    def _update_tools(self):
        ...
        if active_ui == self._y_tool.ui:
            self._y_tool.update()
        if active_ui == self._logger_tool.ui:
